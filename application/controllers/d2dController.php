...
class d2dController extends CI_Controller{

	public function __construct(){
		parent::__construct();
		$this->load->helper('url');
		$this->load->model('d2dModel');
	}

	public function showD2D(){
		$data["getData"] = $this->d2dModel->get_D2D();
		$this->load->view("d2dIndex",$data);
	}

	public function showBodyDetail($bodyID){
		$data["getDataByID"] = $this->d2dModel->get_Data_by_ID($bodyID);
